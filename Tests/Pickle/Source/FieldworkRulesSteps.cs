...
namespace FieldworkCompanions.PickleSteps
{
    ...
    public partial class FieldworkSteps
    {
        ...
        public void Shear(PickleContext ctx, string colonistName)
        {
            ...
            sheep.TryGetComp<CompShearable>().Gathered(pawn);
        }

        /// <summary>
        /// Vanilla gives a cow no obedience to learn, so the state is granted straight on the tracker, which is
        /// all the mod asks (<c>HasLearned</c>), together with a master and the follow box. What it proves is
        /// that the animal being milked is never its own helper, the one gesture where the subject of the work
        /// is itself a candidate.
        /// </summary>
        [Given("Fieldwork Companions: {string} is a milk cow of {string}, granted obedience directly, following at work")]
        public void CowThatCountsAsCompanion(PickleContext ctx, string cowName, string masterName)
        {
            ...
            cow.training.learned[TrainableDefOf.Obedience] = true;
            cow.playerSettings.master = master;
            cow.playerSettings.followFieldwork = true;
            ctx.Require(cow.training.HasLearned(TrainableDefOf.Obedience), $"{cowName} does not read as obedient");
        ...
        public void MinesUntilBond(PickleContext ctx, string colonistName, string animalName, int maxTimes)
        {
            ...
            for (var i = 0; i < maxTimes && !Companions.IsBonded(worker, animal); i++)
            {
                Companions.TryAssist(worker, AssistKind.Mining);
            }
        }
        ...
        private static string DescribeLake(WaterBody body, Map map, IntVec3 cell) =>
            $"type {body.waterBodyType}, {body.Size} cells, population {map.waterBodyTracker.FishPopulationAt(cell)}, "
